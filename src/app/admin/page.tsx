      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-zinc-600">
              Gerencie, edite e abra suas páginas.
            </p>
          </div>
        ...
        {!error && pages.length > 0 && (
          <div className="mt-8 overflow-hidden rounded-2xl border border-zinc-200">
            <div className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-4 border-b border-zinc-200 bg-zinc-50 px-5 py-4 text-sm font-semibold">
              <div>Página</div>
              <div>Status</div>
              <div>Criada em</div>
              <div>Ações</div>
            ...
            {pages.map((page) => (
              <div
                key={page.id}
                className="grid grid-cols-[2fr_1fr_1fr_1fr] gap-4 border-b border-zinc-200 px-5 py-4 last:border-b-0"
              >
                <div>
                  <div className="font-semibold">{page.title}</div>
                  <div className="mt-1 text-sm text-zinc-500">/{page.slug}</div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  <Link
                    href={`/editor/${page.id}`}
                    className="rounded-xl border border-zinc-300 px-3 py-2 text-sm transition hover:bg-zinc-100"
                  >
                    Editar
                  </Link>
