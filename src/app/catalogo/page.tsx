        ...
        <div className="flex flex-wrap items-start justify-between gap-4 rounded-2xl border border-[#f0e7db] bg-[#fbf8f3] p-5">
          <div>
            ...
            <p className="mt-2 text-zinc-600">
              Veja todos os produtos publicados em uma única página.
            </p>
          </div>
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {products.map((product) => {
              ...
              const whatsappUrl = `https://wa.me/${whatsapp}?text=${encodeURIComponent(
                `Olá! Tenho interesse em ${product.title}.`
              )}`;

              return (
                <div
                  key={product.id}
                  className="overflow-hidden rounded-[24px] border border-[#ece4d8] bg-[#fbf8f3]"
                >
